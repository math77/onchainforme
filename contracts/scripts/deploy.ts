...
async function main() {
  ...
  const ASSET_STORE_MAINNET_ADDRESS = "0xCd31e7AAE7306E1ef720B51ab13848DDe7A3Ed47";
  const SVG_START_MAINNET_ADDRESS = "0xEE00c043E6e85a57b0409D8E7693f06AdE5ABc9C";


  const convertedStrMiddle = convert(SVG_MIDDLE);


  const assetStoreContract = await ethers.getContractAt("AssetStore", ASSET_STORE_MAINNET_ADDRESS);
  let saveConverted1Txn = await assetStoreContract.saveAsset(convertedStrMiddle);

  
  const newSvgMiddleAddr = await assetStoreContract.assetAddress(ethers.keccak256(convertedStrMiddle));


  const postersContract = await ethers.deployContract("OnchainForMe", [SVG_START_MAINNET_ADDRESS, newSvgMiddleAddr]);
  await postersContract.waitForDeployment();


  console.log("ADDRESSES");

  //console.log("SVG START ADDR: ", svgStartAddr);
  console.log("SVG MIDDLE ADDR: ", newSvgMiddleAddr);
  console.log("POSTERS: ", postersContract.target);
  console.log("ASSET STORE: ", assetStoreContract.target);

  /*
    LAST DEPLOY (TESTNET)

    SVG START ADDR:  0x35F763457f968839CEB31289Fd63E7D05CED3f2B
    SVG MIDDLE ADDR:  0x3f887814413eDa6c4F0392E11a1B00Fa82F3A55F
    POSTERS:  0x4013E672Ff43E774cb21Eb74190a9c442B89993A
    ASSET STORE:  0x98B45C5CbF880AA747f92713A0c744fEC7fDA148
    

    LAST DEPLOY (MAINNET)
  
    SVG START ADDR:  0xEE00c043E6e85a57b0409D8E7693f06AdE5ABc9C
    SVG MIDDLE ADDR:  0xfA6a1Fd61Ac5B40E7D806336FC4872a606410A02
    POSTERS:  0x87709D347C13a7FF375FAAf224fF1344b776dd92
    ASSET STORE:  0xCd31e7AAE7306E1ef720B51ab13848DDe7A3Ed47


    FIRST DEPLOY (MAINNET)

    SVG START ADDR:  0xEE00c043E6e85a57b0409D8E7693f06AdE5ABc9C
    SVG MIDDLE ADDR:  0xFaC6Ced9Bc7AB23d6FD0beFeE1E12536D6b4D2cB
    POSTERS:  0x27678F4B1657d8D3819a67248bA3D44aea52EAE0
    ASSET STORE:  0xCd31e7AAE7306E1ef720B51ab13848DDe7A3Ed47
